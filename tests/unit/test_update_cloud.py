...
def test_with_yaml_dict(run: mocks.Run, mock_file: mocks.NamedTemporaryFile):
    ...
    definition = {'clouds': {'mycloud': {'type': 'manual', 'endpoint': '1.2.3.4'}}}
    juju.update_cloud('mycloud', definition, client=True)

    assert len(mock_file.writes) == 1
    assert 'type: manual' in mock_file.writes[0]
...
def test_requires_client_or_controller():
    ...
    with pytest.raises(TypeError, match='"client" must be true or "controller" must be specified'):
        juju.update_cloud('mycloud', '/path/to/cloud.yaml')
